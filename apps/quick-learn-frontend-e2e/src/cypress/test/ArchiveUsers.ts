export class ArchiveUsers {
    ...
    cy.get('div.p-4').eq(0);
            cy.get('div.mt-2 > div > p').contains("Deactivated on").should('be.visible');
          }
        });
      }
    ...
    getActivateUserList() {
      ...
      getActivateUserButton() {
        cy.get('.text-lg').contains('Inactive Users').should('be.visible');
        cy.get('div.justify-end > button:nth-child(1)').contains("Activate").eq(0).click();
        cy.get('button.text-white').contains("Yes, I'm sure").click();
      }

      getActivateUserMessage(){
        cy.get('div.Toastify__toast-body').contains("User has been activated successfully").should('be.visible');
      }
      ...
      getDeleteUserMessage(){
        cy.get('div.Toastify__toast-body').contains("User has been permanently deleted").should('be.visible');
      }
