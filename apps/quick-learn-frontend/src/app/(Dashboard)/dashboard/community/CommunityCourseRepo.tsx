...
const CoummintyCourseLayout = () => {
  return (
    ...
    <ul className="mt-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-4 2xl:grid-cols-5">
      {
        data.map((course)=>{
          return <li key={course.id} className="col-span-1 hover:shadow-2xl cursor-pointer shadow-md"><a href={`${RouteEnum.COMMUNITY}/${course.id}`}><CourseCarousel  course={course}/></a></li>
        })
      }
